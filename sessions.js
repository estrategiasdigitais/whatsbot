...
module.exports = class Sessions {
    ...
    static async whatsbot(sessionName) {
        var bot;
        const delay = ms => new Promise(res => setTimeout(res, ms));
        bot = await this.start(sessionName);
    ...
    static async sendText(req, idGroup) {
        var params = {
            sessionName: req.body.sessionName,
            number: idGroup ? idGroup : req.body.number,
            text: req.body.text
        }

        var session = await Sessions.whatsbot(params.sessionName);
        if (session) {
